...
![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_1.PNG)

考虑当n=4时，所有的切割方案如下，我们发现当把4英寸的钢条切割为两段2英寸长的钢条时产生的收益最大。

![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_2.PNG)

当区分1+3切法和3+1切法时，n英寸的钢条有2<sup>n-1</sup>种切割方案。  
我们可以手动得到r<sub>i</sub>，及对应切割方案：

![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_CUT_RESULT1.PNG)
...
![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_CUT_ROD.PNG)

这种实现的递归式为：$T(n) = 1 + \sum\limits_{j=0}^{n-1}T(j).$  
我们可以求得$T(n) = 2^i$，这个时间是非常惊人的，而这么大的原因就是因为包含了非常多的重复计算。下图是递归树，红线圈起来的都是重复计算部分。

![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_3.PNG)

#### 使用动态规划方法

* 带备忘的自顶而下法  
与前面的递归方法类似，但是用了一个数组r[1..n]记录最优解，如果发现r[n]已经有记录，则无需重复计算，直接返回该值。
![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_MEMORIZED_CUT_ROD.PNG)
* 自底而上法  
自底而上法采用子问题的自然顺序，依次求解规模为j=1,2...,n的子问题。
![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_BOTTOM_UP_CUT_ROD.PNG)
...
问题的**子问题图**准确的表达了这些信息：
![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_4.PNG)
...
![钢条切割](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_EXTEND_BOTTOM_UP_CUT_ROD.PNG)
...
![矩阵链乘法](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_MATRIX_CHAIN_ORDER.PNG)
使用m[i,j]存储A<sub>i..j</sub>的最优解的值，s[i,j]存储A<sub>i..j</sub>的最优分割点k。
![矩阵链乘法](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_5.PNG)
4. 利用计算出的信息构造一个最优解  
利用s数组存储的最优分割点，我们可以递归的构造出一个最优解，伪代码如下：  
![矩阵链乘法](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_PRINT_OPTIMAL_PARENS.PNG)
...
1. 刻画一个最优解的结构特点
![矩阵链乘法](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_1_T.PNG)
2. 递归的定义最优解的值
...
\right.$$
3. 计算最优解的值，通常采用自底而上的方法  
![矩阵链乘法](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_LCS_LENGTH.PNG)
c[i, j] 存储序列X<sub>1->i</sub>和Y<sub>1->j</sub>最长公共子序列的长度  
b[i, j] 存储序列X<sub>1->i</sub>和Y<sub>1->j</sub>最优解依赖的子问题的方向
4. 利用计算出的信息构造一个最优解
![矩阵链乘法](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_8.PNG)
当b[i, j]存储的是“↖”时，表示x<sub>i</sub>和y<sub>j</sub>相同。  
所以从b[m, i]开始，训着箭头方向查找“↖”，便可以找到一个最长公共子序列。  
伪代码如下：  
![矩阵链乘法](https://raw.githubusercontent.com/maomao9003/Introduction-to-Algorithms/master/.res/15_PRINT_LCS.PNG)
